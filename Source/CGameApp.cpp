...
void CGameApp::SetupGameState() {
    ...
    D3DXMatrixPerspectiveFovLH(&m_mtxProjection, D3DXToRadian(60.0f), fAspect,
                               1.01f, 1000.0f);
...
//-----------------------------------------------------------------------------
LRESULT CALLBACK CGameApp::StaticWndProc(HWND hWnd, UINT Message, WPARAM wParam,
                                         LPARAM lParam) {
// NOTE: Added 64bit compatibility casts using 'LONG_PTR' to prevent
...
// If this is a create message, trap the 'this' pointer passed in and store it
// within the window.
#pragma warning(push)
#pragma warning(disable : 4244)// inhibit ignorable warning
    if (Message == WM_CREATE)
        SetWindowLongPtr(hWnd, GWLP_USERDATA,
                         (LONG_PTR) ((CREATESTRUCT FAR *) lParam)->lpCreateParams);
#pragma warning(pop)

    // Obtain the correct destination for this message
    CGameApp *Destination =
            (CGameApp *) ((LONG_PTR) GetWindowLongPtr(hWnd, GWLP_USERDATA));
...
//-----------------------------------------------------------------------------
LRESULT CGameApp::DisplayWndProc(HWND hWnd, UINT Message, WPARAM wParam,
                                 LPARAM lParam) {
    float fAspect;
            ...
            switch (LOWORD(wParam)) {
                case ID_ANIM_ROTATION1:
                    ...
                    ::CheckMenuItem(::GetMenu(m_hWnd), ID_ANIM_ROTATION1,
                                    MF_BYCOMMAND | (m_bRotation1) ? MF_CHECKED
                                                                  : MF_UNCHECKED);
                    break;
                ...
                case ID_ANIM_ROTATION2:
                    ...
                    ::CheckMenuItem(::GetMenu(m_hWnd), ID_ANIM_ROTATION2,
                                    MF_BYCOMMAND | (m_bRotation2) ? MF_CHECKED
                                                                  : MF_UNCHECKED);
                    break;
...
bool CGameApp::BuildObjects() {
    ...
    if (m_Mesh.AddPolygon(6) < 0)
        return false;

    // Front Face
    pPoly = m_Mesh.m_pPolygon[0];
    if (pPoly->AddVertex(4) < 0)
        return false;
...
void CGameApp::DrawPrimitive(CPolygon *pPoly, D3DXMATRIX *pmtxWorld) {
    ...
    for (USHORT v = 0; v < pPoly->m_nVertexCount + 1; v++) {
        ...
        vtxCurrent.x =
                vtxCurrent.x * m_nViewWidth / 2 + m_nViewX + m_nViewWidth / 2;
        vtxCurrent.y =
                -vtxCurrent.y * m_nViewHeight / 2 + m_nViewY + m_nViewHeight / 2;

        // If this is the first vertex, continue. This is the first point of our
        // first line.
        if (v == 0) {
...
void CGameApp::AnimateObjects() {
    ...
    if (m_bRotation2) {
        ...
        // Apply the rotation to our object's matrix
        D3DXMatrixMultiply(&m_pObject[1].m_mtxWorld, &mtxRotate,
                           &m_pObject[1].m_mtxWorld);
